...
describe('project handoff ZIP', () => {
  ...
  it('pins the handoff and keeps WebMCP/deployment guidance explicit', () => {
    ...
    expect(deploy).toContain('Never paste either secret')
    expect(deploy).not.toContain('pnpm test\n')
    expect(deploy).toContain('/api/auth/callback/github')
    expect(deploy).toContain('ADMIN_GITHUB_LOGIN')
    expect(deploy).toContain('GITHUB_CLIENT_SECRET')
    expect(deploy).toContain('BETTER_AUTH_SECRET')
    expect(deploy).toContain('/admin/sign-in')
    expect(handoff).toContain('Do not invent a test command')
    expect(handoff).toContain('self-managed GitHub OAuth')
